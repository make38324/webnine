...
// Main theme functions start
    Ventcamp = {
        ...
        /**
         * Helper function to vertically center the element.
         *
         * Checks if element is rendered or not. If element's height is zero,
         * it's probably because element is not rendered yet. This function will
         * try again later.
         *
         * @param $element Pointer to the element
         * @param index Element index
         */
        centerElement: function( $element, index ) {
            // Can't use 'this' here, because it points out to Window object
            var options = Ventcamp.centering;
            ...
            // Check if counter is less than 10, because we don't want this function to run indefinitely
            if ( options.counter.hasOwnProperty(index) && options.counter[index] <= 10 ) {
                // If height equals zero, means that element is not rendered yet
                if ( ( elHeight <= 0 || parentHeight <= 0 ) && options.first_run ) {
                    options.counter[index]++;
                } else {
                    $parent.css({
                        'padding-top': padding, // Set padding-top
                        'padding-bottom': 0     // Set zero padding-bottom
                    });

                    // If it's the first run, clear interval
                    if ( options.first_run ) {
                        // Padding is calculated, clear interval
                        clearInterval( options.interval[index] );
                        // It's not the first run anymore, set flag to false
            ...
            // Only if interval is set
            if ( options.first_run ) {
                // 10 runs are out, clear interval
                clearInterval( options.interval[index] );
                // It's not the first run anymore, set flag to false
                options.first_run = false;
            }
        ...
        centeredBlock: function() {
            $('.vc_row-fluid.centered-content').each(function () {
                ...
                var padding = (rowHeight - containerHeight) / 2;

                $(this).css({ 'padding-top': padding, 'padding-bottom': padding });
            });

            var centeringElements = $('.centered-block');

            // Foreach every element that needs to be centered
            for ( var index = 0; index < centeringElements.length; ++index ) {
                // Only it's the first run, then set interval. Otherwise just call a function
                if ( this.centering.first_run ) {
                    // Call function every 20 ms
                    this.centering.interval[index] = setInterval( this.centerElement, 20, $( centeringElements[index] ), index );
                    // Init out counter
                    this.centering.counter[index]  = 0;
                } else {
                    // Most likely it's window resize event, just call a function
                    this.centerElement( $( centeringElements[index] ), index );
                }
            }
        ...
        windowResizeHandler: function (event) {
            ...
            this.centering.first_run = false;
            this.centeredBlock();
